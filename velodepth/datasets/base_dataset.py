...
import velodepth.datasets.pipelines as pipelines
from velodepth.utils import (eval_3d, eval_depth, identity, is_main_process,
...
class BaseDataset(Dataset):
    ...
    def _augmentation_space(self):
        ...
        self.pre_augmentations_dict = {
            "TemporalFlip": pipelines.RandomSequenceFlip(prob=0.25)
        }
    ...
    def pre_augment(self, results):
        for name, aug in self.pre_augmentations_dict.items():
            results = aug(results)
        return results
    ...
    def generate_of(self, results):
        ...
        for t in range(0, T - 1):
            prev_frame = (
                (255.0 * results[(t, 0)]["image"].squeeze(0))
                .permute(1, 2, 0)
                .byte()
                .numpy()
            )
            curr_frame = (
                (255.0 * results[(t + 1, 0)]["image"].squeeze(0))
                .permute(1, 2, 0)
                .byte()
                .numpy()
            )
            flow_bwd = cv2.DISOpticalFlow_create(cv2.DISOPTICAL_FLOW_PRESET_FAST).calc(
                cv2.cvtColor(curr_frame, cv2.COLOR_RGB2GRAY),
                cv2.cvtColor(prev_frame, cv2.COLOR_RGB2GRAY),
                None,
            )  # (H, W, 2) flow field

            flow_bwd = torch.from_numpy(flow_bwd).permute(2, 0, 1)
            results[(t, 0)]["flow_bwd"] = flow_bwd.unsqueeze(0)
            results[(t, 0)].get("gt_fields", set()).add("flow_bwd")

        results[(T - 1, 0)].get("gt_fields", set()).add("flow_bwd")
        results[(T - 1, 0)]["flow_bwd"] = torch.zeros_like(
            results[(T - 2, 0)]["flow_bwd"]
        )
        return results
