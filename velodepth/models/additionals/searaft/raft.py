...
class RAFT(
    ...
    def forward(self, image1, image2, iters=None):
        ...
        info = info_predictions[-1]
        info[:, 2] = torch.clamp(info[:, 2], min=0, max=self.args.var_max)
        info[:, 3] = torch.clamp(info[:, 3], min=self.args.var_min, max=0)
        uncertainty = (info[:, :2]  * info[:, 2:]).sum(dim=1, keepdim=True) - 0.5 * math.log(ori_W ** 2 + ori_H ** 2)

        return {'flow': flow_predictions[-1], 'info': uncertainty, 'nf': None}